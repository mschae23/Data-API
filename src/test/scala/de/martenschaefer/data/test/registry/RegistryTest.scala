...
class RegistryTest extends UnitSpec {
    ...
    "A RegistryElementCodec" should "encode objects in the registry correctly" in {
        ...
        intCodec.encodeElement(6) shouldBe Success(Element.StringElement("test:six"))
    }

    it should "encode objects not in the registry correctly" in {
        intCodec.encodeElement(7) shouldBe Success(Element.IntElement(7))
    }
    ...
    it should "decode objects in the registry correctly" in {
        ...
        intCodec.decodeElement(Element.StringElement("test:eight")) shouldBe Success(8)

        intCodec.decodeElement(Element.IntElement(8)) shouldBe Success(8)
    }

    it should "decode objects not in the registry correctly" in {
        intCodec.decodeElement(Element.StringElement("test:nine")) shouldBe
            Failure(Vector(RecordParseError.NotAnInt(Element.StringElement("test:nine"), List())))

        intCodec.decodeElement(Element.IntElement(9)) shouldBe Success(9)
    }
}
